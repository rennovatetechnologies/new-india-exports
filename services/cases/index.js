...
/** Live catalog stages for this case (previous plans ∪ current). */
...
/** True when every current catalog stage has been marked complete. */
...
/**
 * Attach live plan catalog KYC + stages on the response. Testing: no case snapshot writes.
 */
async function withWorkflowStages(doc) {
  if (!doc) return null;
  const stages = await buildWorkflowStagesForCase(doc);
  const kycDocs = await liveKycDocsForCase(doc);
